mr_mean_ivw <- function(d) {
  ...
  weights2 <- sqrt(
    ((se_out^2 + stats::coefficients(ivw1)[1]^2 * se_exp^2) / b_exp^2)^-1
  )
...
mr_mean_egger <- function(d) {
  ...
  weights2 <- sqrt(
    ((se_out^2 + stats::coefficients(egger1)[2]^2 * se_exp^2) / b_exp^2)^-1
  )
  ...
  eggeroutliers <- dplyr::tibble(
    ...
    Qj = weights2^2 *
      (ratios -
        stats::coefficients(egger2)[1, 1] / weights2 -
        stats::coefficients(egger2)[2, 1])^2,
    Qpval = stats::pchisq(Qj, 1, lower.tail = FALSE)
  )
  ...
  directional_pleiotropy <- dplyr::tibble(
    ...
    se = c(
      egger2$coefficients[1, 2] / max(1, egger2$sigma),
      egger2$coefficients[1, 2]
    ),
    pval = stats::pt(abs(b / se), nrow(d) - 2, lower.tail = FALSE) * 2
  )
  ...
  if (!is.null(m[[1]])) {
    ...
    if (nrow(dat_st) == 0) {
      ...
      if ("outliers" %in% names(m[[3]])) {
        temp <- subset(
          dat_st,
          !SNP %in% subset(m[[3]]$outliers, Qpval < 0.05)$SNP
        )
