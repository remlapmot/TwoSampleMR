...
mv_extract_exposures_local <- function(
  ...
  stopifnot(
    inherits(filenames_exposure, "character") |
      inherits(filenames_exposure, "list")
  )
...
mv_harmonise_data <- function(
  ...
  expname <- subset(
    exposure_dat,
    !duplicated(id.exposure),
    select = c(id.exposure, exposure)
  )
  outname <- subset(
    outcome_dat,
    !duplicated(id.outcome),
    select = c(id.outcome, outcome)
  )
  ...
  for (i in 1:nexp) {
    ...
    if (intercept) {
      if (instrument_specific) {
        ...
        mod <- summary(stats::lm(
          marginal_outcome[index, i] ~ beta.exposure[index, i]
        ))
      } else {
        marginal_outcome[, i] <- stats::lm(
          beta.outcome ~ beta.exposure[, -c(i), drop = FALSE]
        )$res
      ...
      if (instrument_specific) {
        ...
        mod <- summary(stats::lm(
          marginal_outcome[index, i] ~ 0 + beta.exposure[index, i]
        ))
      } else {
        marginal_outcome[, i] <- stats::lm(
        ...
        mod <- summary(stats::lm(
          marginal_outcome[, i] ~ 0 + beta.exposure[, i]
        ))
      }
    }
    ...
    if (plots) {
      p[[i]] <- ggplot2::ggplot(
        ...
        ggplot2::labs(
          x = paste0("SNP effect on ", nom2[i]),
          y = "Marginal SNP effect on outcome"
        )
    }
  }
  result <- data.frame(
...
mv_basic <- function(mvdat, pval_threshold = 5e-8) {
  ...
  for (i in 1:nexp) {
    ...
    # Get outcome effects adjusted for all effects on all other exposures
    marginal_outcome[, i] <- stats::lm(
      beta.outcome ~ beta.exposure[, -c(i)]
    )$res
...
mv_ivw <- function(mvdat, pval_threshold = 5e-8) {
  ...
  for (i in 1:nexp) {
    ...
    # Get the effect of the exposure on the residuals of the outcome
    mod <- summary(stats::lm(
      beta.outcome[index] ~ 0 + beta.exposure[index, ],
      weights = w[index]
    ))
...
mv_subset <- function(
  ...
  instruments <- apply(
    mvdat$exposure_pval,
    1,
    function(x) any(x < pval_threshold)
  )
