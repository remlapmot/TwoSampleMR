...
mr_rucker_internal <- function(dat, parameters = default_parameters()) {
  ...
  if (parameters$test_dist == "z") {
    pval_ivw_fe <- stats::pnorm(abs(b_ivw_fe / se_ivw_fe), lower.tail = FALSE) *
      2
  } else {
    pval_ivw_fe <- stats::pt(
      abs(b_ivw_fe / se_ivw_fe),
      nsnp - 1,
      lower.tail = FALSE
    ) *
      2
  ...
  if (parameters$test_dist == "z") {
    pval_ivw_re <- stats::pnorm(
      abs(
        stats::coefficients(mod_ivw)[1, 1] / stats::coefficients(mod_ivw)[1, 2]
      ),
      lower.tail = FALSE
    ) *
      2
  ...
  pval1_egger_fe <- stats::pt(
    abs(b1_egger_fe / se1_egger_fe),
    nsnp - 2,
    lower.tail = FALSE
  ) *
    2
  ...
  if (parameters$test_dist == "z") {
    pval0_egger_fe <- stats::pnorm(
      abs(b0_egger_fe / se0_egger_fe),
      lower.tail = FALSE
    ) *
      2
  } else {
    pval0_egger_fe <- stats::pt(
      abs(b0_egger_fe / se0_egger_fe),
      nsnp - 2,
      lower.tail = FALSE
    ) *
  ...
  if (parameters$test_dist == "z") {
    pval0_egger_re <- stats::pnorm(
      stats::coefficients(mod_egger)[1, 1] /
        stats::coefficients(mod_egger)[1, 2],
      lower.tail = FALSE
    )
  } else {
  ...
  intercept$CI_low <- intercept$Estimate -
    stats::qnorm(1 - alpha / 2) * intercept$SE
  intercept$CI_upp <- intercept$Estimate +
    stats::qnorm(1 - alpha / 2) * intercept$SE
...
mr_rucker_bootstrap <- function(dat, parameters = default_parameters()) {
  ...
  l <- list()
  for (i in 1:nboot) {
    dat2$beta.exposure <- stats::rnorm(
      nsnp,
      mean = dat$beta.exposure,
      sd = dat$se.exposure
    )
    dat2$beta.outcome <- stats::rnorm(
      nsnp,
      mean = dat$beta.outcome,
      sd = dat$se.outcome
    )
  ...
  rucker_median$P <- 2 *
    stats::pt(
      abs(rucker_median$Estimate / rucker_median$SE),
      nsnp - 1,
      lower.tail = FALSE
    )
  ...
  rucker_mean$P <- 2 *
    stats::pt(
      abs(rucker_mean$Estimate / rucker_mean$SE),
      nsnp - 1,
      lower.tail = FALSE
    )
  ...
  p2 <- ggplot2::ggplot(modsel, ggplot2::aes_string(x = "Estimate")) +
    ggplot2::geom_density(
      ggplot2::aes_string(fill = "model_name"),
      alpha = 0.4
    ) +
    ggplot2::geom_vline(
      data = res,
...
mr_rucker_jackknife_internal <- function(
  dat,
  ...
  if (nrow(dat) < 15) {
    ...
    p1 <- ggplot2::ggplot(
      bootstrap,
      ggplot2::aes_string(x = "Q", y = "Qdash")
    ) +
      ggplot2::geom_point(ggplot2::aes_string(colour = "model")) +
      ggplot2::geom_point(data = subset(bootstrap, i == "Full")) +
      ggplot2::scale_colour_brewer(type = "qual") +
