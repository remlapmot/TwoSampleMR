...
knit_report <- function(input_filename, output_filename, ...) {
  ...
  } else stop("Please choose a filename with pdf, html, docx or md suffix")
}
...
mr_report <- function(
  ...
  for (i in seq_len(nrow(combinations))) {
    ...
    tablist <- lapply(
      ...
      function(x) {
        if (is.null(x)) {
          ...
          subset(
            x,
            id.exposure == combinations$id.exposure[i] &
              id.outcome == combinations$id.outcome[i],
    ...
    plotlist <- lapply(p, function(x) {
      ...
      index <- which(
        d$id.exposure == combinations$id.exposure[i] &
          d$id.outcome == combinations$id.outcome[i]
      )
      if (length(index) < 1) {
    ...
    output_file[i] <- knit_report(
      file.path(path, "mr_report.Rmd"),
      output_file[i],
      ...
    )
  }
