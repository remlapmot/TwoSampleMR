...
add_metadata <- function(
  dat,
  cols = c("sample_size", "ncase", "ncontrol", "unit", "sd")
) {
  ...
  get_info <- function(id, what = "exposure", cols) {
    ...
    names(info)[names(info) == paste0("sample_size.", what)] <- paste0(
      "samplesize.",
      what
    )
    if ("sample_size" %in% cols) {
      index <- grepl("ukb-d", info$id) &
        is.na(info[[paste0("samplesize.", what)]])
