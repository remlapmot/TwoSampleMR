...
mr_singlesnp <- function(
  ...
  res <- plyr::ddply(dat, c("id.exposure", "id.outcome"), function(X) {
    ...
    for (i in seq_along(all_method)) {
      ...
      nom <- c(
        nom,
        paste0("All - ", subset(mr_method_list(), obj == all_method[i])$name)
      )
    }
...
mr_forest_plot <- function(singlesnp_results, exponentiate = FALSE) {
  res <- plyr::dlply(
    ...
    function(d) {
      ...
      levels(d$SNP)[
        levels(d$SNP) == "All - Inverse variance weighted"
      ] <- "All - IVW"
...
mr_density_plot <- function(
  ...
  res <- plyr::dlply(
    ...
    function(d) {
      ...
      d1 <- subset(
        mr_results,
        id.exposure == d2$id.exposure[1] & id.outcome == d2$id.outcome[1]
      )
      ...
      ggplot2::ggplot(d2, ggplot2::aes(x = b)) +
        ...
        ggplot2::geom_point(
          y = 0,
          colour = "red",
          ggplot2::aes(size = 1 / se)
        ) +
        ggplot2::geom_vline(
          data = mr_results,
          ggplot2::aes(xintercept = b, colour = method)
        ) +
