...
mr_meta_fixed_simple <- function(b_exp, b_out, se_exp, se_out, parameters) {
  if (
    sum(!is.na(b_exp) & !is.na(b_out) & !is.na(se_exp) & !is.na(se_out)) < 2
  ) {
    return(list(b = NA, se = NA, pval = NA, nsnp = NA))
  }
...
mr_meta_fixed <- function(b_exp, b_out, se_exp, se_out, parameters) {
  if (
    sum(!is.na(b_exp) & !is.na(b_out) & !is.na(se_exp) & !is.na(se_out)) < 1
  ) {
    return(list(
      b = NA,
      se = NA,
    ...
    return(
      1 /
        ...
        1 /
          2 *
          sum(
            (b_out - param[length(b_exp) + 1] * param[seq_along(b_exp)])^2 /
              se_out^2
          )
    )
  }
...
linreg <- function(x, y, w = rep(x, 1)) {
  ...
  bhat <- stats::cov(x * w, y * w, use = "pair") /
    stats::var(x * w, na.rm = TRUE)
...
#' \item{dat}{Original data used for MR Egger regression}
#' }
mr_egger_regression_bootstrap <- function(
  b_exp,
  b_out,
  se_exp,
  se_out,
...
weighted_median_bootstrap <- function(
  b_exp,
  b_out,
  se_exp,
  se_out,
