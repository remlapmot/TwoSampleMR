...
mr_mode <- function(
  ...
  beta <- function(BetaIV.in, seBetaIV.in, phi) {
    ...
    s <- 0.9 *
      (min(stats::sd(BetaIV.in), stats::mad(BetaIV.in))) /
      length(BetaIV.in)^(1 / 5)
  ...
  boot <- function(BetaIV.in, seBetaIV.in, beta_Mode.in, nboot) {
    ...
    for (i in 1:nboot) {
      ...
      penalty <- stats::pchisq(
        weights *
          (BetaIV.boot - beta.boot[i, (length(phi) + 1):(2 * length(phi))])^2,
        df = 1,
        lower.tail = FALSE
      )
  ...
  beta_WeightedMode <- beta(
    BetaIV.in = BetaIV,
    seBetaIV.in = seBetaIV[, 1],
    phi = phi
  )
  weights <- 1 / seBetaIV[, 1]^2
  penalty <- stats::pchisq(
    weights * (BetaIV - beta_WeightedMode)^2,
    df = 1,
    lower.tail = FALSE
  )
  pen.weights <- weights * pmin(1, penalty * parameters$penk) # penalized

  beta_PenalisedMode <- beta(
    BetaIV.in = BetaIV,
    seBetaIV.in = sqrt(1 / pen.weights),
    phi = phi
  )
  ...
  beta_WeightedMode_NOME <- beta(
    BetaIV.in = BetaIV,
    seBetaIV.in = seBetaIV[, 2],
    phi = phi
  )
...
#' \item{pval}{p-value}
#' }
mr_weighted_mode_nome <- function(
  b_exp,
  b_out,
  se_exp,
  se_out,
...
mr_mode_broken <- function(
  ...
  res <- plyr::ddply(
    dat,
    c("id.exposure", "exposure", "id.outcome", "outcome"),
    function(x) mr_mode_internal(x, parameters, mode_method = "all")
  )
  if (mode_method != "all") {
