...
harmonise_data <- function(exposure_dat, outcome_dat, action = 2) {
  ...
  for (i in seq_len(nrow(combs))) {
    x <- subset(
      res.tab,
      id.exposure == combs$id.exposure[i] & id.outcome == combs$id.outcome[i]
    )
    message(
      "Harmonising ",
      x$exposure[1],
    ...
    attr(x, "log")[['candidate_variants']] <- sum(
...
check_required_columns <- function(dat, type = "exposure") {
  required_columns <- c(
    "SNP",
    paste0(
      c("id.", "", "beta.", "se.", "effect_allele.", "other_allele."),
      type
    )
  )
