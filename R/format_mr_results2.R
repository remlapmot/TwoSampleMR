...
subset_on_method <- function(
  ...
  dat <- subset(
    mr_res,
    (nsnp == 1 & method == single_snp_method) |
      (nsnp > 1 & method == multi_snp_method)
  )
  return(dat)
}
...
combine_all_mrresults <- function(
  ...
  if (any(Class == "factor")) {
    Pos <- which(
      unlist(lapply(names(res), FUN = function(x) class(res[, x]))) == "factor"
    )
  ...
  if (any(Class == "factor")) {
    Pos <- which(
      unlist(lapply(names(het), FUN = function(x) class(het[, x]))) == "factor"
    )
    for (i in seq_along(Pos)) {
  ...
  if (any(Class == "factor")) {
    Pos <- which(
      unlist(lapply(names(sin), FUN = function(x) class(sin[, x]))) == "factor"
    )
    for (i in seq_along(Pos)) {
  ...
  res <- merge(
    res,
    het,
    by = c("id.outcome", "id.exposure", "Method"),
    all.x = TRUE
  )
  res <- plyr::rbind.fill(
    res,
  ...
  res <- res[, c(
    names(res)[names(res) %in% Cols],
    names(res)[which(!names(res) %in% Cols)]
  )]
  ...
  if (method == 2) {
    ...
    for (i in seq_along(id.set.unique)) {
      ...
      for (j in seq_along(id.subset.unique)) {
        # print(j)
        print(paste(
          "identifying best powered summary set: ",
          id.subset.unique[j],
          sep = ""
        ))
        ...
        if (any(is.na(dat2$ncase.outcome)))
          stop(paste(
            "number of cases missing for summary set: ",
            id.subset.unique[j],
            sep = ""
          ))
        ...
        if (dist.outcome == "binary") {
          if (any(is.na(n.cas)) || any(is.na(n.con))) {
            ...
            iv.se <- 1 /
              sqrt(mean(dat2$samplesize.outcome, na.rm = TRUE) * r2sum)
          } else {
            iv.se <- 1 /
              sqrt(
                mean(n.cas, na.rm = TRUE) * mean(n.con, na.rm = TRUE) * r2sum
              ) #standard error of the IV should be proportional to this
          }
        }
    ...
    # }
    dat2 <- dat2[,
      !names(dat2) %in% c("iv.se", "power.prune", "id.set", "id.subset")
    ]
    dat <- dat2
...
size.prune <- function(dat) {
  ...
  id.keep <- id.expout[
    !duplicated(paste(dat$exposure, dat$originalname.outcome))
  ]
