...
format_data <- function(
  ...
  if (inherits(dat, "data.table")) {
    ...
    stop(paste0(
      "Your ",
      datname,
      " data.frame is also of class 'data.table', ",
      "please reformat as simply a data.frame with ",
      datname,
      " <- data.frame(",
      datname,
      ") and then rerun your format_data() call."
    ))
  }
  ...
  if (!is.na(i)) {
    ...
    index <- !is.finite(dat$eaf.outcome) |
      dat$eaf.outcome <= 0 |
      dat$eaf.outcome >= 1
    index[is.na(index)] <- TRUE
  ...
  if (!is.na(i)) {
    ...
    if (is.logical(dat$effect_allele.outcome)) {
      dat$effect_allele.outcome <- substr(
        as.character(dat$effect_allele.outcome),
        1,
        1
      )
    }
  ...
  if (!is.na(i)) {
    ...
    if (is.logical(dat$other_allele.outcome)) {
      dat$other_allele.outcome <- substr(
        as.character(dat$other_allele.outcome),
        1,
        1
      )
    }
  ...
  if (!is.na(i)) {
    ...
    index <- !is.finite(dat$pval.outcome) |
      dat$pval.outcome < 0 |
      dat$pval.outcome > 1
    index[is.na(index)] <- TRUE
  ...
  if (
    ...
    dat$pval.outcome <- stats::pnorm(
      abs(dat$beta.outcome) / dat$se.outcome,
      lower.tail = FALSE
    ) *
      2
  ...
  if (samplesize_col %in% names(dat)) {
    ...
    "ncontrol.outcome" %in% names(dat) && "ncase.outcome" %in% names(dat)
  ) {
    message("Generating sample size from ncase and ncontrol")
...
format_gwas_catalog <- function(gwas_catalog_subset, type = "exposure") {
  message(
    "This function is now deprecated and has been replaced by 'format_data'."
  )
  return(NULL)
}
...
format_gtex_eqtl <- function(gtex_eqtl_subset, type = "exposure") {
  ...
  gtex_eqtl_subset[[type]] <- paste0(
    gtex_eqtl_subset$gene_name,
    " (",
    gtex_eqtl_subset$tissue,
    ")"
  )
...
format_aries_mqtl <- function(aries_mqtl_subset, type = "exposure") {
  ...
  aries_mqtl_subset$Phenotype <- paste0(
    aries_mqtl_subset$cpg,
    " (",
    aries_mqtl_subset$age,
    ")"
  )
...
random_string <- function(n = 1, len = 6) {
  ...
  for (i in 1:n) {
    randomString[i] <- paste(
      sample(c(0:9, letters, LETTERS), len, replace = TRUE),
      collapse = ""
    )
  }
  return(randomString)
...
combine_data <- function(x) {
  stopifnot(is.list(x))
  if ("exposure" %in% names(x[[1]])) type <- "exposure" else if (
    "outcome" %in% names(x[[1]])
  )
    type <- "outcome" else stop("Datasets must be generated from format_data")
