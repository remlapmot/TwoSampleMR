...
format_1_to_many <- function(
  ...
  mr_res[, TraitM] <- paste(
    Letters[seq_along(mr_res[, TraitM])],
    mr_res[, TraitM]
  )
...
sort_1_to_many <- function(
  ...
  if (sort_action == 1) {
    ...
    mr_res$Index <- unlist(lapply(
      seq_along(unique(mr_res[, group])),
      FUN = function(x)
        rep(
          Letters[Letters == Letters[x]],
          length(which(mr_res[, group] == groups[x]))
        )
    ))
  ...
  if (sort_action == 3) {
    ...
    mr_res$b.sort <- NA
    mr_res1 <- mr_res[
      mr_res[, group] %in% mr_res[, group][duplicated(mr_res[, group])],
    ]
    mr_res2 <- mr_res[
      !mr_res[, group] %in% mr_res[, group][duplicated(mr_res[, group])],
    ]

    mr_res1$b.sort[mr_res1[, trait_m] == priority] <- mr_res1[, b][
      mr_res1[, trait_m] == priority
    ]
    # mr_res1$b.sort[mr_res1[,group]==priority]<-1000
    for (i in unique(mr_res1[, group])) {
      mr_res1$b.sort[
        mr_res1[, group] == i & is.na(mr_res1$b.sort)
      ] <- mr_res1$b.sort[mr_res1[, group] == i & !is.na(mr_res1$b.sort)]
    }
...
forest_plot_basic2 <- function(
  ...
  null_line <- ifelse(
    all.equal(dat$effect - dat$lo_ci, dat$up_ci - dat$effect) == TRUE,
    0,
    1
  )
