system_metrics <- function(dat) {
  ...
  if (nrow(dat) > 2) {
    ...
    metrics$shap_ivw <- stats::shapiro.test(stats::residuals(
      ruck$lmod_ivw
    ))$statistic
    metrics$shap_egger <- stats::shapiro.test(stats::residuals(
      ruck$lmod_egger
    ))$statistic
    metrics$ks_ivw <- stats::ks.test(
      stats::residuals(ruck$lmod_ivw),
      "pnorm"
    )$statistic
    metrics$ks_egger <- stats::ks.test(
      stats::residuals(ruck$lmod_egger),
      "pnorm"
    )$statistic
  }
  return(metrics)
...
mr_moe_single <- function(res, rf) {
  ...
  res$estimates$method2 <- paste(
    res$estimates$method,
    "-",
    res$estimates$selection
  )
  res$estimates <- dplyr::left_join(
    res$estimates,
    pred,
    by = c("method2" = "method")
  ) %>%
