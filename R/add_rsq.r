...
add_rsq_one <- function(dat, what = "exposure") {
  ...
  if (!paste0("rsq.", what) %in% names(dat)) {
    dat[[paste0("pval.", what)]][
      dat[[paste0("pval.", what)]] < 1e-300
    ] <- 1e-300
    if (compareNA(dat[[paste0("units.", what)]][1], "log odds")) {
      ...
      all(grepl("SD", dat[[paste0("units.", what)]])) &&
        all(!is.na(dat[[paste0("eaf.", what)]]))
    ) {
      dat[[paste0("rsq.", what)]] <- NA
      ...
      ind1 <- !is.na(dat[[paste0("pval.", what)]]) &
        !is.na(dat[[paste0("samplesize.", what)]])
      dat[[paste0("rsq.", what)]] <- NA
      if (sum(ind1) > 0) {
        ...
        dat[[paste0("effective_n.", what)]] <- dat[[paste0(
          "samplesize.",
          what
        )]]
      } else {
...
test_r_from_pn <- function() {
  ...
  for (i in seq_len(nrow(param))) {
    ...
    y <- x *
      sqrt(param$rsq[i]) +
      scale(stats::rnorm(param$n[i])) * sqrt(1 - param$rsq[i])
    param$rsq_emp[i] <- stats::cor(x, y)^2
    param$pval[i] <- max(
      stats::coefficients(summary(stats::lm(y ~ x)))[2, 4],
      1e-300
    )
...
get_r_from_pn <- function(p, n) {
  optim.get_p_from_rn <- function(x, sample_size, pvalue) {
    abs(
      -log10(suppressWarnings(get_p_from_r2n(x, sample_size))) - -log10(pvalue)
    )
  }
...
contingency <- function(af, prop, odds_ratio, eps = 1e-15) {
  ...
  y <- vapply(
    z,
    function(a)
      zapsmall(matrix(c(a, prop - a, af - a, 1 + a - af - prop), 2, 2)),
    matrix(0.0, 2, 2)
  )
