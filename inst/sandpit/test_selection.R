find_invalid_instruments <- function(d1, d2, d3, steiger_thresh = 0.05) {
  ...
  d$remove_confounder <- with(
    d,
    ux & uy * ux_p < steiger_thresh & uy_p < steiger_thresh
  )
...
get_summary_stats <- function(pop1, pop2, popu) {
  ...
  x$inst <- y$inst <- u$inst <- rep(
    c("x", "y", "u"),
    c(ncol(pop1$G1), ncol(pop1$G2), ncol(pop1$Gu))
  )
