...
for (i in 1:100) {
  ...
  res4[i] <- with(
    bp3,
    mr_ivw(beta.exposure, beta.outcome, se.exposure, se.outcome)
  )$pval
}
...
for (i in seq_len(nrow(param))) {
  effs <- make_effs(
    ninst1 = param$nsnp[i],
    var_xy = param$var_xy[i],
    var_g1x = param$var_g1x[i],
    mu_g1y = param$mu_g1y[i]
  )
...
for (i in 1:BootSim) {
  ...
  if (weights == 2) {
    W = 1 / (seBetaYG^2 / BXG^2 + (BYG^2) * seBetaXG^2 / BXG^4)
  }
  ...
  if (
    QQ[i] >= qchisq(1 - alpha, DF1) && QQ[i] - QQd[i] >= qchisq(1 - alpha, 1)
  ) {
    Mod[i] = 3
  }
  if (
    QQ[i] >= qchisq(1 - alpha, DF1) &&
      QQ[i] - QQd[i] >= qchisq(1 - alpha, 1) &&
      QQd[i] >= qchisq(1 - alpha, DF2)
  ) {
    Mod[i] = 4
  }
}
